...
class Bicycle():
    ...
    def move(self, time_of_motion, vel, delta):
        ...
        while self.time < time_of_motion:
            self.beta = np.arctan((self.lf * np.tan(self.delta)) / (self.lr + self.lf))
            vel =  Twist()

            vel.linear.x = self.vel * np.cos(self.pose.theta)
            vel.linear.y = self.vel * np.sin(self.pose.theta)
            vel.linear.z = 0
